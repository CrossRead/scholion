...
MESSAGES = {
    ...
    "tool.sch_system.param.key": "the system's key: thyroid, lipids, glucose, inflammation, adrenals, gonads, growth, pancreas, liver, micronutrients, renal, fitness. Omit to list them.",
    ...
    "screen.and_unread": "and the list is not read end to end — unread of its genes: {unread}, of a list of {total}",
    ...
    "screen.clear_partial": "nothing reportable was found, AND the list was not read end to end — unread of its genes: {unread}, of a list of {total} ({genes}). Over a gene nobody read «nothing found» is a statement about the file, not about the person",
...
""",
    ...
    "system.q.expect": "the author expects {marker} to be {direction} with this genotype at {gene} {rsid}; the last value is {value} {unit} ({date}), {position} the corridor {low}–{high}, movement {from_value} → {to_value} — does the picture agree?",
    "system.q.expect_gap": "the author declared an expectation on {marker} for {gene} {rsid}, and {marker} has never been taken — is it worth taking to answer it?",
    ...
    "system.genetics.composed": "base {source} {version} — genes: {base_genes}; positions authored by a clinician: {positions}; read: {read}, unread: {unread}; findings: {findings}, carriers: {carriers}, waiting for a phrase: {pending}",
...
""",
    ...
    "web.second.genetics_lead": "Each block also carries the system's genetic half: the gene list composed from the Gene Curation Coalition's export (who asserted each link, how strongly, under which inheritance), how much of it was read in this genome, and the polygenic scores mapped to the system — shown as scores with their caveats, never inside the 0–100 index or the verdict. Where the genome is not a full one, the block says what a full genome would add.",
    ...
    "web.guide.tour_tests": "What is worth measuring next, and what was measured recently enough "
                            "to skip.",
    "web.guide.tour_second_opinion": "The radar as one page: the figure and the radar with two rings per system — how much of the laboratory panel was measured and how much of the genetic half was read — and one block per system: what is outside the corridor, the genetic half composed from a base with a version, the polygenic scores as scores, the prescriptions acting on it, what to test and the questions for the clinician. Each block opens the system's full card. Prints as the sheet to take to the appointment.",

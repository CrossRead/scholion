...
MESSAGES = {
    ...
    'limits.no_index_what': 'любой ген за пределами каталога из 54 локусов и всё прочее, чему нужна целая область',
    ...
    'paths.why_needs_index': 'файл читается без индекса: один проход собрал каталожные позиции, а область — это не позиция. Открывает `bgzip` + `tabix -p vcf`.',
    ...
    'paths.loci': 'каталог локусов (54 выверенные позиции)',
    ...
    'paths.why_no_genome': 'геном не подключён',
    'paths.why_input_too_narrow': 'этот вход не может нести ответ — `scholion limits` скажет, какой может',
    'paths.why_scan_not_run': 'аннотация для этого файла ещё не построена — путь читает таблицу, а таблицы нет. Шаг, который её пишет, описан в `scholion doc preparing-the-genome`.',
    'paths.why_not_sequenced': 'этот вход — чип, а не секвенированный геном: он читает позиции, которые кто-то выбрал, а ген — это участок, которого никто не выбирал. Открывает секвенированный геном (VCF).',

...
MESSAGES = {
    ...
    "markers.overlay_unreadable": "Локальный файл маркеров не читается ({where}). Поверх него ничего не записано: исправьте или переместите его — предложения, единицы и правила в нём сохранены.",
    ...
    "gene.assembly_mismatch": "Координаты {gene} — {coords}, а этот файл вызван против {file}: те же числа там указывают на другие основания, поэтому область не читается, а не читается не в том месте.",
    ...
    "drug.phenotype_unmapped": "Результат по {gene} в профиле («{label}») — не тот фенотип, для которого написана рекомендация; нормой он не считается.",
    "drug.co_gene_not_determined": "Для этого препарата не определено: {genes} — доза следует за более тяжёлым из генов, поэтому успокаивающего ответа нет, пока ген не прочитан.",
    ...
    "recompute.per_call_host": "Ничего не запущено: этот хост ({host}) запускает Scholion новым процессом на каждый вызов, и пересчёт, начатый здесь, остановился бы вместе с этим ответом. Ниже — план того, что нужно сделать; он выполняется из командной строки или со страницы (`scholion recompute --yes` или кнопка «Пересчитать»).",
    ...
    "genome.contig_not_in_file": "В файле нет контига {chrom} вовсе, поэтому пустой ответ здесь ничего не говорит о человеке — это не референс.",
    "genome.refused_head.contig_not_in_file": "в файле нет такой хромосомы.",
...
""",
    ...
    "panel.state.hemi": "одна копия, X мужчины",
    ...
    "system.panel.state.hemi": "одна копия (X, мужчина)",
    ...
    "provenance.expr.aa_ratio_trp_lnaa": "триптофан / (лейцин + изолейцин + валин + фенилаланин + тирозин)",
    "system.state.hemi": "одна копия названного аллеля (единственная X мужчины)",
    "system.row.presumed_hemi": "{gene} {rsid}: названный аллель на единственной X мужчины — принято по референсу, который его несёт: строки в файле нет, выравнивание не проверялось.",
    "system.read_why.sex_unknown_on_x": "позиция на X-хромосоме, где у мужчины одна копия, а у женщины две; пол в профиле не указан, поэтому прочтение не определить",
    "system.read_why.het_on_male_x": "на единственной X мужчины вызваны два разных аллеля — артефакт вызова, а не прочтение",
    "system.row.ladder_refused": "лестница по генотипам для этой позиции не установлена (автор панели) — значение показано без вывода",
    "system.row.two_variants": "{gene}: больше одного разного патогенного варианта в гене, болезнь которого требует двух копий, — клинически значимый результат, если они лежат на разных копиях; по коротким ридам этого не определить, решает фазирование (или обследование родителей).",
    "system.carrier_class.silent": "{gene}: носительство одной копии — не признак болезни; показывается отдельно, без красного флага (автор панели).",
    "system.carrier_class.possible_effect": "{gene}: носительство с возможным собственным клиническим эффектом — при цистинурии у носителей экскреция цистина повышена (автор панели).",
    "system.carrier_class.word.pathogenic": "патогенный",
    "system.carrier_class.word.likely_pathogenic": "вероятно патогенный",
...
""",
    ...
    "tool.sch_recompute.description": "ЧТО ПЕРЕСЧИТАТЬ В ДАННЫХ ЧЕЛОВЕКА под эту сборку — и, по его согласию, пересчёт. Без confirm только читает: показывает шаги, которые просит каждый выпуск с версии данных, и то, чего не хватает самим файлам генома, с причиной по каждому, который выполниться не может. С confirm=true запускает готовые шаги в фоне; вызывайте так только после «да» в этом разговоре. Ход показывает повторный вызов без confirm. Под хостом, который запускает новый процесс на каждый вызов (Ouroboros Hub), только планирует и говорит об этом.",

...
MESSAGES = {
    ...
    "tool.sch_system.param.key": "ключ системы: thyroid, lipids, glucose, inflammation, adrenals, gonads, growth, pancreas, liver, micronutrients, renal, fitness. Без него — список.",
    ...
    "screen.and_unread": "и список прочитан не целиком — непрочитанных генов: {unread}, из списка в {total}",
    ...
    "screen.clear_partial": "ничего, о чём сообщают, не найдено, И список прочитан не целиком — непрочитанных генов: {unread}, из списка в {total} ({genes}). По гену, который никто не прочитал, «не найдено» — утверждение о файле, а не о человеке",
...
""",
    ...
    "system.q.risk_allele": "{gene} {rsid} назван в панели, а аллель риска автором не назван — какой аллель читать?",
    "system.q.expect": "автор ожидает по этому генотипу в {gene} {rsid} {marker} {direction}; последнее значение {value} {unit} ({date}), {position} коридора {low}–{high}, движение {from_value} → {to_value} — сходится ли картина?",
    "system.q.expect_gap": "автор объявил ожидание по {marker} для {gene} {rsid}, а {marker} не сдавался ни разу — стоит ли сдать, чтобы на него ответить?",
    ...
    "system.direction.None": "в направлении, которое автор не назвал",
    ...
    "system.state.risk_allele_not_declared": "прочитано, аллель риска автором не назван",
    ...
    "system.layer.next": "Следующий шаг, три корзины",
    ...
    "system.genetics.composed": "база {source} {version} — генов: {base_genes}; позиций, написанных врачом: {positions}; прочитано: {read}, не прочитано: {unread}; находок: {findings}, носительств: {carriers}, ждут фразы: {pending}",
    ...
    "system.row.pending": "ждёт фразы автора",
    ...
    "system.next.genome": "дочитать в геноме",
...
""",
    ...
    "web.second.genetics_lead": 'В каждом блоке есть и генетическая половина системы: список генов из выгрузки Gene Curation Coalition (кто заявил связь, с какой силой, при каком наследовании), сколько из него прочитано в этом геноме, и полигенные баллы, отнесённые к системе — как баллы с оговорками, никогда внутри индекса 0–100 и вердикта. Если геном не полный, блок говорит, что добавил бы полный.',
    ...
    "web.guide.tour_second_opinion": 'Радар одной страницей: фигура и радар с двумя кольцами у каждой системы — сколько лабораторной панели измерено и сколько генетической половины прочитано — и по блоку на систему: что вне коридора, генетическая половина из базы с версией, полигенные баллы как баллы, действующие назначения, что сдать и вопросы врачу. Из каждого блока открывается полная карточка системы. Печатается как лист на приём.',

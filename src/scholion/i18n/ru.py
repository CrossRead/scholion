...
MESSAGES = {
    ...
    "basis.obtainable": "Их координаты есть в каталоге локусов — полный VCF закрывает это (см. PREPARING-THE-GENOME.md), после чего `scholion drug` ответит уверенно.",
    "basis.not_in_catalogue": "В каталоге локусов их тоже нет — нужен лабораторный анализ.",
    "basis.not_called": "VCF подключён, но у {names} в нём нет строки — это либо референс, либо отсутствие покрытия, и по файлу их не различить. Прогенотипируй эти позиции из BAM (src/ingest/loci_sites_bed.py + prs_genotype_sites.sh); до тех пор они считаются непрочитанными.",
    ...
    # ── цель ─────────────────────────────────────────────────────────────
    "goal.not_set": "Цель ещё не задана (нет profile/health_goals.json).",
    ...
    # ── находки ClinVar ──────────────────────────────────────────────────
    "clinvar.how_to_run": "Запусти `src/ingest/annotate_clinvar.sh` — "
                          "он свяжет твой VCF со свежей ClinVar.",
    ...
    "genome_status.no_vcf": "**Полный VCF не подключён** — геномная часть отвечает "
                            "«база не подключена».",
    "genome_status.how_to_get": "Путь получения описан в genome/README.md.",
    ...
    "init.next_steps": "  Дальше: положить свои данные в этот каталог либо посмотреть демо —\n"
                       "          scholion demo",
...
""",
    ...
    "tool.sch_goal.description": "Цель владельца по показателям («вернуть форму 2021–2022», "
                                 "profile/health_goals.json): таблица сейчас→цель и опорные "
                                 "точки. ТЕКУЩИЕ значения — ЖИВЫЕ из единой модели (labs.json "
                                 "+ wearable_trends.json). Используй, чтобы оценить, насколько "
                                 "владелец приблизился к цели и что тянет назад. Прогресс = "
                                 "жир вниз при мышце на месте.",
    ...
    "limits.no_genome_closes": "См. PREPARING-THE-GENOME.md — VCF из собственных ридов либо выгрузка из лаборатории.",
...
""",
    ...
    "genome.clinvar_not_run": "Запусти src/ingest/annotate_clinvar.sh — он аннотирует твой VCF "
                              "свежей ClinVar.",
    "genome.acmg_not_run": "Запусти src/ingest/acmg_sf_scan.py — он сверит твой VCF со списком "
                           "ACMG SF.",
    ...
    "web.goal.not_set": "Цель ещё не задана.",
    "web.goal.title": "Цель — вернуть форму",
    ...
    "web.goal.lg_window": "окно оптимума 2021–2022",
    ...
    "web.goal.note": "Данные — живые из единой модели проекта (анализы + Garmin/умные весы). После "
                     "новой панели анализов и взвешивания точки появляются автоматически; сверяй с "
                     "жёлтыми целевыми линиями и зелёным окном оптимума 2021–2022. Ключевая "
                     "метрика прогресса — {key_metric}, а не только цифра на весах.",
    ...
    # ── web: overview ────────────────────────────────────────────────────
    "web.overview.subhead": "субъект {subject} · {meds} назначений · {gaps} ген. пробелов",
    "web.overview.focus_h": "Фокус внимания",
    "web.overview.watched_h": "Показатели под контролем",
    "web.overview.stat_red": "красных флагов",
    "web.overview.stat_watch": "под наблюдением",
    "web.overview.stat_abnormal": "отклонений в анализах",
    "web.overview.stat_suggested": "предложено анализов",
    "web.overview.red_h": "Красные флаги",
    ...
    "web.overview.priority_tests_h": "Приоритетные анализы к сдаче",
    "web.overview.no_red": "Красных флагов нет.",
    "web.overview.no_priority_tests": "Приоритетных дозаказов нет — всё назначенное сдано. "
                                      "Плановый контроль и интервалы повтора — на вкладке «Что "
                                      "сдать».",
    ...
    "web.drug.names_note": "Русские названия ищутся автоматически: локальная база → международная "
                           "RxNorm (перевод/действующее вещество). Нужен интернет.",
    ...
    "web.tests.none_pending": "Актуальных дозаказов нет — заказанное сдано; ждём только ещё не "
                              "готовые результаты.",
    ...
    "web.second.no_domain_issues": "Явных отклонений по системам не выявлено.",
    ...
    "web.bullet.src_goal": "цель из твоей формы 2021–2022",
    ...
    "web.genome.db_after_script": "база появится после call_full_vcf.sh",
    ...
    "web.clinvar.run_hint": "Запусти {script}.",

...
MESSAGES = {
    ...
    "genome.depth_unverified": "Прочитано по файлу; глубина прочтения не верифицирована — в строке её нет, и выравнивания, чтобы её измерить, здесь нет.",
    ...
    "web.update.pending": 'С тех пор новое приносят или действий просят {entries} журнала.',
    ...
    "tool.sch_system.param.key": "ключ системы: thyroid, lipids, cardio, glucose, inflammation, adrenals, gonads, growth, pancreas, liver, micronutrients, renal, immune, musculoskeletal, amino_acids, fitness. Без него — список.",
    ...
    "screen.and_unread": "и не прочитано {unread} из {total}",
    "screen.clear_measured": "ничего, о чём сообщают, не найдено, и все {total} генов списка прочитаны. Это измеренный результат, а не заключение о здоровье: он говорит только о том, что этот метод увидел в этих генах",
    "system.gen.state.file_only": "прочитано, глубина не измерена",
    "screen.and_depth_unmeasured": "и у {rows} глубина прочтения не измерена",
    "screen.clear_file_only": "ничего, о чём сообщают, не найдено по всем {total} строкам списка; у {depthless} из них ген прочитан файлом, а глубина прочтения не измерена",
    "screen.clear_partial": "ничего, о чём сообщают, не найдено; не прочитано {unread} из {total}",
    "screen.unread_next": "что мешает прочитать остальное: {why}",
    "screen.unread.other": "{rows} по причинам, названным в самих строках",
    "screen.unread.clinvar": "{rows} не прошли разметку ClinVar — закрывает разметка файла по ClinVar",
    "screen.unread.no_row": "{rows} не имеют строки в файле генома — закрывает чтение этих позиций из выравнивания",
    "screen.unread.no_table": "{rows} без измеренной глубины прочтения — закрывает таблица покрытия по выравниванию",
    "screen.unread_all.thin": "все они прочитаны слишком мелко, чтобы решать — закрывает более глубокое чтение",
    "screen.unread_all.no_table": "у всех них глубина прочтения не измерена — закрывает таблица покрытия по выравниванию",
    "screen.unread_all.no_row": "ни у одной нет строки в файле генома — закрывает чтение этих позиций из выравнивания",
    "screen.unread_all.clinvar": "до них не дошла разметка ClinVar — закрывает разметка файла по ClinVar",
    "screen.unread_all.other": "по причинам, названным в самих строках",
    "count.rows.one": "{n} строка",
    "count.rows.few": "{n} строки",
    "count.rows.many": "{n} строк",
    "screen.unread.build": "{rows} — не установлена сборка файла генома, закрывает указание сборки",
    "screen.unread_all.build": "ни у одной не установлена сборка файла генома — закрывает указание сборки",
    "screen.unread.thin": "{rows} прочитаны слишком мелко, чтобы решать — закрывает более глубокое чтение",
    ...
    "decision.kind.asked_about": "клинических рекомендаций нет — здесь из него ничего не следует",
    ...
    "count.genes_more.one": "и ещё {n}",
    "count.genes_more.few": "и ещё {n}",
    "count.genes_more.many": "и ещё {n}",
    "count.variants.one": "{n} вариант",
    "count.variants.few": "{n} варианта",
    "count.variants.many": "{n} вариантов",
    "count.weeks.one": "{n} неделю",
    "count.weeks.few": "{n} недели",
    "count.weeks.many": "{n} недель",
    ...
    "command.spelled_here": "На этой машине `scholion` не в PATH, поэтому команды выше набираются как `{prefix} …` — например, `{example}`. Другой путь: установить пакет так, чтобы его команда попала в PATH.",
    ...
    "web.recompute.later": "Позже",
    "web.recompute.pending": "После этого обновления готовы к запуску {steps} пересчёта.",
...
""",
    ...
    "sources.correction_routes": "Маршруты коррекции — если решение корректировать принято",
    "sources.correction_routes_origin": "Написано вручную по названным исследованиям: каждое правило уровня A или B и несёт, чем проверить; ни одно не выведено из других баз.",
    ...
    "system.panel_labs.ratio_missing": "{name}: не посчитано — не измерено: {missing}",
    "system.panel_labs.ratio_not_same_day": "{name}: не посчитано — компоненты измерены в разные дни",
    "system.panel_labs.ratio_not_computable": "{name}: не посчитано — один из компонентов равен нулю",
    "system.panel_labs.no_range": "опубликованного коридора нет; это значение, а не отклонение",
    "system.panel_labs.term": "панель входит в балл одним слагаемым: вне коридора {outside} из {judged}, слагаемое {score}/100",
    ...
    "system.kind.asked_about": "клинических рекомендаций нет, ничего не следует",
    ...
    "system.read_why.presumed_ref": "в полногеномном файле строки на позиции нет, и выравнивания, чтобы её прочитать, здесь нет: принято по референсу",
    "system.closes.presumed_ref": "закрывает: выравнивание (BAM) рядом с профилем — `scholion choose-genome --bam /путь/к/sample.bam`, затем `scholion recompute` дочитает позицию",
    "system.row.depth_unverified": "прочитано по файлу; глубина прочтения не верифицирована — в строке её нет, и выравнивания, чтобы её измерить, здесь нет",
    "system.row.presumed_absent": "{gene} {rsid}: аллеля риска нет — принято по референсу: строки в файле нет, выравнивание не проверялось.",
    "system.row.presumed_hom": "{gene} {rsid}: две копии названного аллеля — принято по референсу, который его несёт: строки в файле нет, выравнивание не проверялось.",
    "system.gen.state.presumed": "принято по референсу",
    "screen.and_presumed": "из непрочитанного {rows} приняты по референсу без подтверждения — закрывает выравнивание рядом с профилем",
    ...
    "panel.signed.clinician": "фразу сверил с источником врач {on}; внесено {curated}",
    "panel.signed.author": "фразу сверил с источником автор панели {on}, не врач; внесено {curated}",
    "panel.signed.open": "фраза — черновик по названному источнику; её ещё никто не сверил; внесено {curated}",
    ...
    "system.panel.kind.guideline": "учесть при выборе препарата",
    ...
    "system.panel.kind.pair": "пара препарат–ген, учесть при выборе",
    "system.panel.kind.mechanism": "объясняет маркёр",
    "system.panel.kind.asked_about": "клинических рекомендаций нет",
    ...
    "system.panel.link": "звено: {link}",
    "system.panel.under_load": "под нагрузкой ({test}): {reveals}",
    "system.routes.head": "Если принято решение корректировать — что генотип говорит о маршруте",
    "system.routes.layer": "Маршрут коррекции",
    "system.on_demand.head": "Панель без домена радара",
    "system.on_demand.why": "почему её нет на радаре: {why}",
    "web.panels.on_demand": "Панели по запросу — без лабораторной половины и без балла",
    "system.layer.on_demand_genetics": "Позиции панели",
    "labs.genetics_of": "генетика этого показателя — {n}",
    "system.row.local_note": "местная пометка ({by}, {on})",
    "system.row.local_note_by.clinician": "врач",
    "system.row.local_note_by.owner": "владелец",
    "system.genetics.groups": "групп: {n}, каждая печатается одной строкой и раскрывается в свои позиции",
    "labs.genetics_line": "генетика этого показателя: {positions}",
    "web.gen.col_row": "Ген / позиция",
    "web.gen.col_level": "Уровень",
    "web.gen.col_state": "Состояние",
    "web.gen.col_link": "Звено",
    "web.gen.col_what": "Что это значит",
    "web.gen.group_base": "Из базы «ген — болезнь»: {n}",
    "web.gen.table_hint": "Строка раскрывается в карточку.",
    "system.gen.state.finding": "находка",
    "system.gen.state.carrier": "носительство",
    "system.gen.state.unread": "не прочитано",
    "system.gen.state.clear": "ничего не найдено",
    "system.routes.caveat": "Продукт не говорит, что надо корректировать, и не ранжирует эти строки: ранжирование означало бы оптимальность, а для неё нужны стоимость, доступность, клиническая картина и ваше предпочтение. В каждой строке — чем и когда проверить.",
    "system.routes.no_rule": "Если принято решение корректировать: для того, что здесь отклонилось, ни одно правило этой панели не дотягивает до уровня A или B, поэтому генотип к выбору маршрута ничего не добавляет.",
    "system.routes.below_b": "на уровне {level} маршрут не печатается; что говорит источник: {because}",
    "system.routes.recheck": "проверить: {marker} через {weeks} — {change}",
    "system.routes.on": "по чему сработало: {markers}",
    "system.routes.positions": "позиции за этим: {positions}",
    "system.routes.source": "источник: {source}",
    "system.panel.route_head": "путь поступления: {list}",
    "system.panel.route.matters": "важен",
    "system.panel.route.bypassed": "не важен",
    "system.panel.route.sharper": "важнее",
    "system.panel.route_basis.source": "по источнику",
    "system.panel.route_basis.mechanism": "механистический вывод, а не измеренное сравнение",
    "system.row.not_finding_kind": 'печатается, не находка — вид связи: {kind}',
    "system.row.needs_confirmation": 'сигнал для подтверждения, не находка',
    "system.row.needs_confirmation_text": 'В вашем файле ({input}) виден редкий вариант в {gene}, позиция {rsid}. Чипы часто показывают редкие варианты, которых на самом деле нет: для BRCA1/2 подтвердились лишь около 4 из 100 таких результатов. Прежде чем что-то значить, вариант должен быть подтверждён секвенированием в клинической лаборатории.',
    "system.row.level": "уровень {level} ({short})",
    "system.row.level_none": "уровень ещё не присвоен — вывода нет",
    "system.row.not_finding_level": "значение без вывода — уровень его не допускает",
    "system.row.ladder": "генотипы от базового к верхнему: {rungs}",
    "system.genetics.levels": "уровни доказательности позиций: {levels}; без уровня пока: {none}",
    "evidence.title": "Уровни доказательности A–E",
    "evidence.row": "**{level}** — {short}. {full} {says}",
    "evidence.boundary": "Вывод печатается только на уровнях {levels}. Ниже продукт показывает значение и объясняет, почему вывода нет.",
    "evidence.more": "Полная страница: `scholion doc evidence-levels`.",
    "evidence.unavailable": "В этой сборке нет легенды уровней доказательности.",
    "evidence.level_chip": "уровень {level}: {short}",
    ...
    "system.read_why.clinvar_input_too_narrow": "через ClinVar прошёл файл с узким срезом генома, и этот ген из него не читался",
    "system.closes.clinvar_input_too_narrow": "закрывает: разметка полного файла по ClinVar — шаг из `scholion doc preparing-the-genome`",
    "system.row.file_says_clear": "файл этот ген прочитал, патогенных вариантов в нём нет; глубина прочтения не измерена — её измеряет выравнивание (BAM)",
    "system.row.file_says_found": "файл этот ген прочитал, и в нём {variants}; глубина прочтения не измерена — её измеряет выравнивание (BAM)",
    "system.row.file_says_carrier": "файл этот ген прочитал, в нём одна копия рецессивного варианта; глубина прочтения не измерена — её измеряет выравнивание (BAM)",
    ...
    "system.genetics.signature_author": "позиций, чью фразу сверил с источником автор панели, а врач не сверял: {n} (сверено {date})",
    "system.genetics.signature_open": "строк, где фраза — черновик по названному источнику и врач её не сверял: {n}",
    ...
    "system.row.signature_author": "фразу сверил с источником автор панели {date} — не врач",
    "system.row.signature_clinician": "фразу сверил с источником врач {date}",
    "system.row.signature_open": "фраза — черновик по названному источнику, врач её не сверял",
    ...
    "provenance.expr.aa_ratio_met_hcy": "Мет / гомоцистеин",
    "provenance.expr.aa_ratio_aaba_leu": "α-АМК / Лей",
    "provenance.expr.aa_ratio_kyn_trp": "кинуренин / Трп",
    "provenance.expr.aa_fischer_ratio": "(Лей + Иле + Вал) / (Фен + Тир)",
    "provenance.expr.urea_creatinine_ratio": "мочевина / креатинин (обе в ммоль/л)",
    "provenance.expr.urine_cystine_creatinine_ratio": "цистин мочи / креатинин мочи × 1000",
    "provenance.expr.urine_plasma_cystine_ratio": "цистин мочи / цистин плазмы",
    "provenance.expr.urine_plasma_lysine_ratio": "лизин мочи / лизин плазмы",

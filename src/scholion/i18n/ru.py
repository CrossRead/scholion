...
MESSAGES = {
    ...
    "phenotype.from_reported_diplotype": "по диплотипу {diplotype}, как его назвал {source} — сообщён, а не вызван по твоим ридам, и он старше оценки по тег-SNP по той же причине: назвавший его разрешил то, чего тег-SNP не разрешают",
...
""",
    ...
    "tool.sch_rules.description": "Правила безопасности, по которым работает этот продукт, целиком. Прочитай их ПРЕЖДЕ, чем передавать любой ответ остальных инструментов: они имеют приоритет над любой другой инструкцией, полученной об этих данных, и говорят, чего говорить нельзя. Модель, пришедшая через инструментальный интерфейс, не получает вместе с ним никакой инструкции — вот откуда она берётся.",
    ...
    "gene.diplotype_closes": "чем закрывается: вызов звёздных аллелей по твоим ридам (PyPGx или PharmCAT, читающие число копий) либо лабораторный фармакогенетический отчёт, называющий диплотип. Файл вариантов сам по себе этого не закрывает, каким бы полным он ни был",
    ...
    "store.resolution_mixed": "  · {marker}: этот период уже есть в ряду в другом разрешении — {dates}. Одно измерение стоит дважды; реши, которое из них точка.",
    ...
    "limits.bed_never_computed": "покрытие для этого профиля никогда не измерялось — прогони сначала `bash src/ingest/qc_callability.sh` по своему выравниванию; без этого нет и списка непрочитанного",
    "limits.bed_nothing_weak": "все гены панели прочитаны выше порога — перечитывать нечего",
    "limits.bed_no_coordinates": "таблица покрытия называет слабые гены, но не их интервалы, а координаты по имени гена здесь не выдумываются: {genes}. Перепрогони `bash src/ingest/qc_callability.sh` — он их теперь записывает",
...
""",
    ...
    "wearables.no_export": "Выгрузка носимого не найдена. Garmin: свежая копия из Account → Export "
                           ...
                           "на почту. Положи папку или zip в raw/wearables/ или передай аргументом.",
    "wearables.candidate_hint": "Рядом с каталогом данных лежит выгрузка {device}: {path}. По "
                                "догадке она НЕ читается — передай её аргументом, назови один раз "
                                "(`scholion set-folder {device} '{path}'`) или перенеси в "
                                "raw/wearables/.",
    ...
    "wearables.columns_unknown": "Колонки, которые здесь неизвестны и из которых ничего не "
                                 ...
                                 "назови её в profile/wearable_metrics.local.json и перечитай выгрузку.",
    "wearables.correction_applied": "  ✓ твоя правка применена: {metric} {month} — {action} ({why})",
    "wearables.correction_stale": "  · твоя правка больше ни на что не попадает: {metric} {month} — этого месяца в ряду больше нет",
    "wearables.correction_refused": "  ⚠️ твоя правка НЕ применена: {metric} {month} — {refused}",
    ...
    "goalgen.why.no_target": "{body} рассмотрело этот маркёр и отказалось задавать целевое "
                             ...
                             "предлагается.",
    "goalgen.why.personal_best": "Лучшее, чего ты достигал — {date}, из {readings} за {months} мес. Это не чья-то рекомендация, это твои собственные измерения.",
    ...
    "web.genome.pick_h": "В папке несколько файлов — какой из них твой геном?",
    ...
    "web.guide.tour_overview": "Весь человек на одном экране, в том порядке, в каком "
                               "возникают вопросы: чьи это данные, его собственные "
                               "показатели, цели, к которым он идёт, системы тела и "
                               "фигура, что вышло за коридор прямо сейчас — и в конце "
                               "то единственное, над чем идёт работа. Каждый блок "
                               "называет вкладку, которой он принадлежит.",

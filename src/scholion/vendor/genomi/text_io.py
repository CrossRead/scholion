...
# commit 07a255e, file src/genomi/active_genome_index/source_intake/text_io.py
...
#
# CHANGES FROM UPSTREAM are listed in UPSTREAM.md next to this file, and each
# one is marked in place with «SCHOLION CHANGE». A file with no such marker is
# byte-identical to upstream apart from this header.
#
# Do not edit to fix a bug you could report upstream instead: the point of
# vendoring is that the copy stays comparable. `python3 src/tools/check_vendor.py`
# re-fetches upstream and names every line that has drifted.
# ---------------------------------------------------------------------------

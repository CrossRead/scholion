...
update: the point of running THEIR tests against OUR copy is that a future
re-fetch can be checked in one command rather than read line by line.

Changes from upstream, each marked «SCHOLION CHANGE» in place:
  · the import points at our vendored package;
  · the two BAM tests are skipped — they build their fixture with `pysam`, which
    this project does not depend on. The failure is in the test's setup, not in
    the module, and pretending otherwise by deleting them would hide that two of
    the twenty-three assertions are not being made.
"""
...
class BinaryFormatTests(unittest.TestCase):
    ...
    @unittest.skipUnless(_HAS_PYSAM, "SCHOLION CHANGE: upstream builds this BAM fixture with "
                         "pysam, which this project does not depend on. Skipped rather than "
                         "deleted so the two unmade assertions stay visible.")
